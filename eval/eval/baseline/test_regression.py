...
against the checked-in results/baseline.json. Fails loudly if:
  ...
  (b) val loss regresses beyond tolerance vs. the recorded baseline.
...
# How much worse val loss is allowed to get before we call it a regression,
# rather than normal run-to-run noise. Wider than train-loss tolerance
# because val loss is measured on only 220 pairs -- genuinely noisier.
...
def test_regression():
    ...
    print(f"baseline val_loss:  {baseline['final_val_loss']:.4f}")
    print(f"current  val_loss:  {metrics['final_val_loss']:.4f}")
    print(f"baseline train_loss: {baseline['final_train_loss']:.4f}")
    print(f"current  train_loss: {metrics['final_train_loss']:.4f}")
    ...
    regression = metrics["final_val_loss"] - baseline["final_val_loss"]
    assert regression < VAL_LOSS_TOLERANCE, (
        f"Regression detected: val loss got worse by {regression:.4f} nats "
        f"(tolerance is {VAL_LOSS_TOLERANCE}). Baseline: {baseline['final_val_loss']:.4f}, "
        f"current: {metrics['final_val_loss']:.4f}."
    )

    print("\nPASS -- no regression vs. checked-in baseline.")
